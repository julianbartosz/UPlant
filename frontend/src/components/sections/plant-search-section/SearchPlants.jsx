    ...
    const fetchPlants = async (searchTerm = '') => {
        ...
        try {
            ...
            const response = await fetch(url, {
                method: 'GET',
                credentials: 'include', // outside headers
                headers: {
...
const PlantList = ({ plants, draggable, onPlantClick }) => {
    if (!Array.isArray(plants)) {
        console.error("plants is not an array:", plants);
        return <div className="no-results">Invalid plant data</div>;
    }
    ...
    return (
        <div className="scrollable-section">
            {plants.map((plant) => (
                <div className="plant-item-container" key={plant.id}>
                    <PlantItem
                        plant={plant}
                        draggable={draggable} 
                        onPlantClick={onPlantClick} 
...
const PlantItem = ({ plant, draggable, onPlantClick }) => {
    ...
    if (!plant) {
        console.error("Received null or undefined plant");
        return null;
    }
    
    // Get plant icon based on family or use default
    const plantIcon = plantFamilyIcons[plant.family] || plantFamilyIcons['default'];
